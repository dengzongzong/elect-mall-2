  ...
  <div class="goods-detail">
    <div class="wrapper_1200 acea-row">
        ...
        <div class="acea-row row-top">
          ...
          <div class="text-wrapper">
            ...
            <div v-else class="acea-row row-middle money-wrapper">
              <div class="money-wrap">
                <del>售价：￥{{ realPriceData.price }}</del>
                <div class="acea-row row-middle">
                  <div class="price">
                    <span class="fs-14">到手价：</span>￥<span>{{
                      realPriceData.real_price
                    }}</span>
                  </div>
                  <div
                    v-if="
            ...
            <div
              ...
              v-if="[0, 3].includes(storeInfo.virtual_type)"
            >
              ...
              <div class="counter-wrap">
                <div class="counter">
                  <button
                    ...
                    :disabled="count <= storeInfo.min_qty || !stock"
                    @click="minus"
                  ></button>
                ...
                <span v-if="storeInfo.min_qty > 1"
                  >{{ storeInfo.min_qty }} {{ storeInfo.unit_name }}起购
                </span>
              </div>
            </div>
            <div class="number-wrapper acea-row" v-else>
              <div class="label">库存数量:</div>
              <div class="counter-wrap">
                <span>{{ stock }} {{ storeInfo.unit_name }}</span>
                <span v-if="storeInfo.min_qty > 1"
                  >({{ storeInfo.min_qty }} {{ storeInfo.unit_name }}起购)</span
                >
              </div>
            </div>
          ...
          <div class="detail-bd">
            ...
            <div v-show="tabIndex === 2">
              <div class="acea-row">
                <div
                  class="spec-item"
                  v-for="(item, index) in storeInfo.params_list"
                  :key="index"
                >
      ...
      <div v-if="goodsList && goodsList.length" class="recom-section">
        ...
        <div class="list">
          <div
            v-for="(item, index) in goodsList.slice(0, 5)"
            :key="index"
            @click="goDetail(item)"
            class="item"
          >
...
export default {
    ...
    productAttr: {
      ...
      handler(attr) {
        ...
        if (this.productValue) {
          for (const [k, v] of Object.entries(this.productValue)) {
            if (v && String(v.is_default_select) === "1") {
              def = k;
              break;
            }
          }
        ...
        if (def) {
          ...
          this.attrSelected = parts.slice(0, attr.length);
        } else {
      ...
      handler(attr) {
        if (attr.length) {
          ...
          if (value) {
            ...
            this.count = 1;
            this.realPrice(this.$route.params.id, this.unique);
          } else {
            this.stock = 0;
            this.count = 0;
          }
        ...
        this.count = this.storeInfo.min_qty || 1;
      },
    },
  },
  async asyncData({ app, params, query }) {
    ...
    return {
      ...
      specUnique: goods.data.spec_unique,
      ...
      meta: [
        {
          hid: "keywords",
          name: "keywords",
          content: this.storeInfo.keyword,
        },
        {
          hid: "description",
          name: "description",
          content: this.storeInfo.store_info,
        },
      ],
    };
  },
  ...
  created() {
    ...
    if (!this.productAttr.length) {
      this.realPrice(this.$route.params.id, this.specUnique);
    }
  },
  mounted() {
  ...
  methods: {
    ...
    inputNum() {
      ...
      this.count =
        parseInt(this.count) <= this.storeInfo.min_qty
    ...
    realPrice(id, unique) {
      this.$axios.get(`/product/real_price/${id}/${unique}`).then((res) => {
    ...
    minus() {
      if (this.count > this.storeInfo.min_qty) {
        this.count--;
      }
    },
    plus() {
